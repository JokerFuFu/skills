# routines — bb-browser 驱动的论坛/社媒每日 routine

本类别把若干网站(NodeSeek / x.com / chiphell)用 **[bb-browser](https://github.com/epiral/bb-browser)** 「CLI 化」,封装成一组可被 Claude Code 调用、也可定时执行的 **skill**。每个 skill 一个子目录,**自包含**(带上自己依赖的脚本与 bb-browser 适配器),`bash install.sh` 装好适配器即可用。
...
```
routines/
├── install.sh            # 一键装适配器到 ~/.bb-browser/sites/ + 拉社区 twitter 取数适配器
├── nodeseek-daily/   nodeseek-review/
├── x-ai-daily/   x-reply-draft/   x-research/
└── chiphell-daily/   chiphell-rescue/
```

每个 skill 目录:`SKILL.md`(提示词)+ `scripts/`(取数/发帖脚本)+ `adapters/`(bb-browser 适配器副本);部分含 `data/`(配置模板)。
...
把需要的 skill 目录(如 `nodeseek-daily`)软链或复制到 `~/.claude/skills/`(全局)或某项目的 `.claude/skills/`(项目级),Claude Code 即可按 description 自动触发;也可在自带 routines / 定时任务里按各 SKILL.md 重建。
